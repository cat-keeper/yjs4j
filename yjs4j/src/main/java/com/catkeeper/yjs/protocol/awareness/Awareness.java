package com.catkeeper.yjs.protocol.awareness;

import com.alibaba.fastjson2.JSON;
import com.alibaba.fastjson2.JSONObject;
import com.catkeeper.yjs.utils.Doc;
import com.catkeeper.yjs.utils.Observable;
...
/**
 * Awareness 实现了 Yjs 的用户感知协议（对应 y-protocols/awareness）。
 *
 * <p>Awareness 用于共享非持久化的临时状态，典型场景：
 * <ul>
 *   <li>光标位置 — 显示其他用户正在编辑的位置</li>
 *   <li>用户信息 — 用户名、头像、在线状态</li>
 *   <li>选区范围 — 其他用户选中的文本区域</li>
 * </ul>
 *
 * <p>与文档数据不同，Awareness 状态不会持久化，也不使用 CRDT。
 * 每个客户端维护自己的本地状态，通过定时广播保持活跃。
 * 超过 30 秒未更新的远程客户端会被自动标记为离线。
 *
 * <p>使用示例：
 * <pre>{@code
 * Awareness awareness = new Awareness(doc);
 * awareness.setLocalState(JSONObject.of("user", "Alice", "cursor", 42));
 *
 ...
 * }</pre>
 *
 * <p>对应 JS 版本的 y-protocols/awareness。
 *
 * @author zbs
