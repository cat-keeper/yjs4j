...
import cn.timegap.yjs.structs.ContentType;
import cn.timegap.yjs.structs.Item;
import cn.timegap.yjs.utils.DSDecoder;
import cn.timegap.yjs.utils.DSEncoder;
import cn.timegap.yjs.utils.Doc;
import cn.timegap.yjs.utils.Transaction;
import cn.timegap.yjs.utils.encoding.EncodingUtil;
import cn.timegap.yjs.utils.event.YArrayEvent;

import java.util.*;

/**
 * YArray 是 Yjs 的共享数组类型，支持任意类型的元素（包括嵌套的共享类型）。
 *
 * <p>主要操作：
 * <ul>
 *   <li>{@link #insert(long, Object...)} — 在指定位置插入一个或多个元素</li>
 *   <li>{@link #push(Object...)} — 在末尾追加元素</li>
 *   <li>{@link #delete(long, long)} — 删除指定范围的元素</li>
 *   <li>{@link #get(int)} — 获取指定位置的元素</li>
 *   <li>{@link #toArray()} — 转为普通 List</li>
 * </ul>
 *
 * <p>支持嵌套类型：
 * <pre>{@code
 * YArray<Object> arr = doc.getArray("list");
 * arr.insert(0, "text", 42, true);       // 基本类型
 * arr.insert(3, new YMap<>());            // 嵌套共享类型
 * }</pre>
 *
 * <p>对应 JS 版本的 Y.Array。
 *
