package cn.timegap.yjs.types;

import cn.timegap.yjs.structs.*;
...
/**
 * AbstractType 是所有 Yjs 共享数据类型的基类。
 *
 * <p>Yjs 提供了几种共享数据类型，它们都继承自 AbstractType：
 * <ul>
 *   <li>{@link YText} — 共享富文本，支持格式化属性</li>
 *   <li>{@link YArray} — 共享数组，支持任意类型元素</li>
 *   <li>{@link YMap} — 共享 Map，支持嵌套类型</li>
 *   <li>{@link YXmlElement} / {@link YXmlFragment} / {@link YXmlText} — 共享 XML 结构</li>
 * </ul>
 *
 * <p>每个 AbstractType 内部维护了两种数据结构：
 * <ul>
 *   <li><b>链表</b>（start 指针）：用于有序内容（YText 的字符、YArray 的元素），
 *       通过 Item.left/right 形成双向链表</li>
 *   <li><b>Map</b>（map 字段）：用于键值对内容（YMap 的属性、YXmlElement 的属性），
 *       每个 key 指向最新的 Item</li>
 * </ul>
 *
 * <p>AbstractType 还提供了事件机制：
 * <ul>
 *   <li><b>observe</b>：监听直接修改事件（如 YMap 的 key 变化）</li>
 *   <li><b>observeDeep</b>：监听深层修改事件（如嵌套 YMap 内部的变化）</li>
 * </ul>
 *
 * <p>对应 JS 版本的 AbstractType 类（types/AbstractType.js）。
 *
 * @author zbs
