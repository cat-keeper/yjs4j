package cn.timegap.yjs.structs;

import cn.timegap.yjs.types.AbstractType;
import cn.timegap.yjs.types.ArraySearchMarker;
...
/**
 * Item 是 Yjs 文档的基本构建块，代表文档中的一段内容。
 *
 * <p>Yjs 的文档本质上是一个双向链表，每个 Item 是链表中的一个节点。Item 存储了：
 * <ul>
 *   <li><b>唯一标识</b>：(clientId, clock) 组成的 ID，全局唯一</li>
 *   <li><b>位置信息</b>：origin（创建时左边的 Item ID）和 rightOrigin（创建时右边的 Item ID），
 *       用于在并发插入时确定正确的位置</li>
 *   <li><b>链表指针</b>：left/right 指向当前的左右邻居</li>
 *   <li><b>内容</b>：content 字段持有实际数据（字符串、数字、嵌套类型等）</li>
 *   <li><b>父节点</b>：parent 指向所属的共享类型（YText、YArray 等）</li>
 * </ul>
 *
 * <p>Item 的关键操作：
 * <ul>
 *   <li><b>integrate</b>：将远程收到的 Item 插入到本地文档的正确位置。这是 YATA 算法的核心，
 *       通过比较 origin 和 clientId 来解决并发插入冲突。</li>
 *   <li><b>delete</b>：标记删除（tombstone），不会从链表中移除，保证后续操作能找到正确位置。</li>
 *   <li><b>gc</b>：垃圾回收，将已删除的 Item 压缩为 GC 节点。</li>
 *   <li><b>mergeWith</b>：将相邻的同类型 Item 合并，减少内存占用。</li>
 * </ul>
 *
 * <p>对应 JS 版本的 Item 类（structs/Item.js）。
 *
 * @author zbs
 */
