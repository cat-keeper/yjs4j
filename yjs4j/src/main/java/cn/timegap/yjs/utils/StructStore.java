...
/**
 * StructStore 是文档的底层存储引擎，管理所有的 Item 和 GC 节点。
 *
 * <p>内部按 clientId 分组，每个 client 的操作按 clock 递增排列在一个 LinkedList 中。
 * 这种结构使得：
 * <ul>
 *   <li>通过 (clientId, clock) 可以快速定位任意 Item（二分查找）</li>
 *   <li>状态向量（每个 client 的最新 clock）可以快速计算</li>
 *   <li>增量更新可以高效生成（只发送对方缺失的 Item）</li>
 * </ul>
 *
 * <p>关键方法：
 * <ul>
 *   <li>{@link #addStruct} — 添加新的 Item/GC 到存储</li>
 *   <li>{@link #find} — 根据 ID 查找 Item</li>
 *   <li>{@link #getState} — 获取指定 client 的最新 clock</li>
 *   <li>{@link #getStateVector(StructStore)} — 获取完整的状态向量</li>
 *   <li>{@link #getItemCleanStart} — 获取 Item 并在必要时拆分（用于精确定位）</li>
 * </ul>
 *
 * <p>对应 JS 版本的 StructStore（utils/StructStore.js）。
 *
 * @author zbs
