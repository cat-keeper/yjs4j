...
import cn.timegap.yjs.types.YText;
    ...
    public List<EventOperator> getDelta() {
        if (this.delta == null) {
            ...
            Transaction.transact(y, (transaction) -> {
                ...
                addOp(params);
                // 移除末尾的 retain（如果无属性）
                while (!params.delta.isEmpty()) {
                    EventOperator last = params.delta.get(params.delta.size() - 1);
                    if (last.getRetain() != null && last.getAttributes() == null) {
                        params.delta.remove(params.delta.size() - 1);
                    } else {
                        break;
                    }
                }
    ...
    private static void addOp(OpParams params) {
        ...
        switch (params.action) {
            case "delete":
                ...
                break;
            case "insert":
                if (params.insert != null && (!(params.insert instanceof String) || !((String) params.insert).isEmpty())) {
                    op = new EventOperator();
                    op.setInsert(params.insert);
                    if (!params.currentAttributes.isEmpty()) {
                        Map<String, Object> attrs = new HashMap<>();
                        params.currentAttributes.forEach((key, value) -> {
                            if (value != null) {
                                attrs.put(key, value);
                            }
                        });
