package cn.timegap.yjs.utils.encoding;

import cn.timegap.yjs.structs.AbstractStruct;
import cn.timegap.yjs.structs.GC;
import cn.timegap.yjs.structs.Item;
import cn.timegap.yjs.structs.Skip;
import cn.timegap.yjs.types.AbstractType;
import cn.timegap.yjs.utils.*;


import cn.timegap.yjs.utils.lib0.Binary;
...
/**
 * EncodingUtil 是 Yjs 的二进制编解码核心，负责将文档状态序列化为字节数组以及反序列化。
 *
 * <p>这是实现跨客户端同步的关键模块。编码格式与 JS 版本完全兼容，
 * 意味着 Java 端和 JavaScript 端可以互相同步。
 *
 * <p>核心方法：
 * <ul>
 *   <li>{@link #encodeStateAsUpdate} — 将文档状态编码为更新消息（可指定目标状态向量，只编码差异部分）</li>
 *   <li>{@link #applyUpdate} — 将收到的更新消息应用到本地文档</li>
 *   <li>{@link #encodeStateVector} — 编码状态向量（每个 client 的最新 clock）</li>
 *   <li>{@link #decodeStateVector} — 解码状态向量</li>
 * </ul>
 *
 * <p>同步流程：
 * <pre>
 * 1. A 发送 encodeStateVector(docA) 给 B
 * 2. B 收到后调用 encodeStateAsUpdate(docB, stateVectorA) 生成差异更新
 * 3. A 收到更新后调用 applyUpdate(docA, update) 应用
 * </pre>
 *
 * <p>支持 V1 和 V2 两种编码格式，V2 更紧凑但计算开销稍大。
 *
 * <p>对应 JS 版本的 encoding.js（utils/encoding.js）。
 *
 * @author zbs
