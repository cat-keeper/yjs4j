...
import cn.timegap.yjs.structs.AbstractStruct;
import cn.timegap.yjs.structs.Item;
import cn.timegap.yjs.types.AbstractType;
import cn.timegap.yjs.types.ItemTextListPosition;
import cn.timegap.yjs.utils.encoding.EncodingUtil;
import cn.timegap.yjs.utils.event.YEvent;

import java.util.*;
import java.util.function.Function;

/**
 * Transaction（事务）是 Yjs 中所有修改操作的执行上下文。
 *
 * <p>每次对文档的修改（插入、删除、格式化等）都在事务中执行。事务的核心作用：
 * <ul>
 *   <li><b>批量操作</b>：事务内的多个修改会被合并为一次更新，减少网络消息和事件触发次数</li>
 *   <li><b>变更追踪</b>：记录哪些类型被修改了（changed），用于触发 observer 回调</li>
 *   <li><b>状态快照</b>：记录事务前后的状态向量（beforeState/afterState），用于生成增量更新</li>
 *   <li><b>删除集</b>：收集事务中所有的删除操作，统一处理</li>
 * </ul>
 *
 * <p>事务的生命周期：
 * <ol>
 *   <li>transact() 创建事务，记录 beforeState</li>
 *   <li>执行用户的修改操作</li>
 *   <li>cleanupTransactions() 触发 observer 回调、发送更新、执行 GC 等</li>
 * </ol>
 *
 * <p>事务支持嵌套：内层 transact() 调用会复用外层的事务对象，只有最外层事务结束时才触发清理。
 *
 * <p>对应 JS 版本的 Transaction 类和 transact() 函数。
 *
 * @author zbs
...
public class Transaction {
    ...
    private static void cleanupTransactions(List<Transaction> transactionCleanups, int i) {
        if (i < transactionCleanups.size()) {
            ...
            try {
                ...
                // 3.2 changedParentTypes 深度回调 (pushed as a closure that adds more callbacks to fs)
                fs.add(() -> {
                    // deep observe events
    ...
    /**
     * Calls all functions in fs. Only throws after all functions were called.
     * Equivalent to lib0's callAll function.
     * Uses index-based iteration so that new elements pushed during execution are also processed.
     */
    private static void callAll(List<Runnable> fs) {
        callAll(fs, 0);
    }
