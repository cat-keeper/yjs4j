...
/**
 * YEvent 是所有 Yjs 事件的基类，描述了某个共享类型上发生的变更。
 *
 * <p>当共享类型被修改时（插入、删除、更新），Yjs 会创建对应的事件对象并传递给 observer 回调。
 * 事件提供了以下信息：
 * <ul>
 *   <li>{@link #target} — 发生变更的共享类型</li>
 *   <li>{@link #getKeys()} — Map 类型的键变更（add/update/delete）</li>
 *   <li>{@link #getChanges()} — 完整的变更信息（added/deleted/delta）</li>
 *   <li>{@link #getPath()} — 从 observeDeep 的根到 target 的路径</li>
 * </ul>
 *
 * <p>子类包括 YArrayEvent、YMapEvent、YTextEvent、YXmlEvent。
 *
 * <p>对应 JS 版本的 YEvent（utils/YEvent.js）。
 *
 * @author zbs
            ...
            for (Item item = target.getStart(); item != null; item = (Item) item.right) {
                if (item.isDeleted()) {
                    ...
                    if (this.adds(item)) {
                        ...
                        Collections.addAll((List<Object>) lastOp.insert, item.content.getContent());
                        added.add(item);
                    } else {
