...
/**
 * UndoManager 提供撤销/重做功能，跟踪指定共享类型上的本地修改。
 *
 * <p>UndoManager 通过记录每次事务的删除集（deletions）和插入集（insertions）来实现撤销：
 * <ul>
 *   <li><b>undo</b>：恢复 deletions 中的 Item，删除 insertions 中的 Item</li>
 *   <li><b>redo</b>：反向操作</li>
 * </ul>
 *
 * <p>特性：
 * <ul>
 *   <li>只跟踪本地修改，远程用户的修改不会进入撤销栈</li>
 *   <li>支持 captureTimeout，在指定时间内的连续修改会合并为一次撤销操作</li>
 *   <li>支持 trackedOrigins，只跟踪特定 origin 的事务</li>
 *   <li>支持 scope，可以同时跟踪多个共享类型</li>
 * </ul>
 *
 * <p>使用示例：
 * <pre>{@code
 ...
 * }</pre>
 *
 * <p>对应 JS 版本的 UndoManager（utils/UndoManager.js）。
 *
 * @author zbs
