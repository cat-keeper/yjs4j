...
/**
 * Snapshot 代表文档在某个时间点的状态快照。
 *
 * <p>快照由两部分组成：
 * <ul>
 *   <li><b>sv</b>（状态向量）：记录每个 client 在快照时刻的最新 clock</li>
 *   <li><b>ds</b>（删除集）：记录快照时刻哪些 Item 已被删除</li>
 * </ul>
 *
 * <p>通过这两个信息，可以判断任意 Item 在快照时刻是否"可见"：
 * 一个 Item 可见当且仅当它的 clock 小于对应 client 的 sv 值，且不在 ds 中。
 *
 * <p>主要用途：
 * <ul>
 *   <li>历史版本查看：{@link #createDocFromSnapshot} 从快照恢复完整文档</li>
 *   <li>差异对比：{@code text.toDelta(snapshot)} 获取快照时刻的文本内容</li>
 *   <li>版本比较：{@link #equalSnapshots} 判断两个快照是否相同</li>
 * </ul>
 *
 * <p>注意：使用快照功能需要关闭 GC（{@code new Doc(new DocOptions() {{ gc = false; }})}），
 * 否则被删除的 Item 会被回收，导致无法恢复历史状态。
 *
 * <p>对应 JS 版本的 Snapshot（utils/Snapshot.js）。
 *
 * @author zbs
