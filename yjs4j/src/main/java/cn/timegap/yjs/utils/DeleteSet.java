...
/**
 * DeleteSet 记录文档中哪些 Item 被删除了。
 *
 * <p>Yjs 的删除是"标记删除"（tombstone）——Item 不会从链表中移除，只是被标记为 deleted。
 * DeleteSet 用一种紧凑的方式记录这些删除信息：按 clientId 分组，每组是一系列
 * (clock, length) 区间，表示从 clock 开始连续 length 个操作被删除。
 *
 * <p>DeleteSet 的用途：
 * <ul>
 *   <li>在事务中收集所有删除操作</li>
 *   <li>作为同步消息的一部分发送给其他客户端</li>
 *   <li>用于快照（Snapshot）中记录某个时间点的删除状态</li>
 * </ul>
 *
 * <p>对应 JS 版本的 DeleteSet（utils/DeleteSet.js）。
 *
 * @author zbs
